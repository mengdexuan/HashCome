...
public class YunpianSMSProvider implements SMSProvider {
    ...
    public MessageResult sendInternationalMessage(String content,  String areaCode, String phone) throws IOException, DocumentException {
        content = String.format("Your verification code is %s, it will be valid within ten minutes, if you are not operating it, please ignore it.", content);
        Map<String, String> params = new HashMap<String, String>();
        params.put("apikey", apikey);
        params.put("text", content);
        params.put("mobile", "+" + areaCode + phone);
        String resultXml= HttpSend.yunpianPost(gateway, params);
        log.info("result = {}", resultXml);
        return parseXml(resultXml);
    }
