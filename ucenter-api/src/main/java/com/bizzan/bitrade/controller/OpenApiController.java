...
public class OpenApiController extends BaseController {
    ...
    public MessageResult saveApiKey(@SessionAttribute(SESSION_MEMBER) AuthMember member, MemberApiKey memberApiKey) throws Exception {
        log.info("-------新增API-key:"+ JSONObject.toJSONString(memberApiKey));
        String pwd = memberApiKey.getPwd();
        Assert.isTrue(StringUtils.isNotEmpty(pwd),"Please enter your login password");

        Member memberResult = memberService.findOne(member.getId());
        isTrue(Md5.md5Digest(pwd + memberResult.getSalt()).toLowerCase().equals(memberResult.getPassword()), msService.getMessage("PASSWORD_ERROR"));
    ...
    public MessageResult updateApiKey(@SessionAttribute(SESSION_MEMBER) AuthMember member,MemberApiKey memberApiKey) throws Exception {
